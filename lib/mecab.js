...
function scriptDir() {
    ...
    return scriptPath.substr(0, scriptPath.replace(/[?#].*/, "").lastIndexOf("/") + 1);
}

var lib;
var instance;
const libPromise = LoadMecab({ locateFile: (path) => scriptDir() + path });
libPromise.then(x => { 
    lib = x;
...
class Mecab 
{
    ...
    static query(str) {
        ...
        let out_length = lib.lengthBytesUTF8(str) * 30;
        ...
        for (let line of ret.split('\n')) {
            ...
            result[result.length] = { pos, pos_detail1, pos_detail2, pos_detail3, conjugation1, conjugation2, dictionary_form, reading, pronunciation };
        }
